...
if (
  ...
  $sql = "SELECT id FROM totaldata WHERE id = ?";
  if (isset(db_fetch_data($link, $sql, [$_POST['id']])[0])) {
    exit();
  }

  $sql = "INSERT INTO totaldata SET id = ?, name_sportsmen = ?, weight_sportsmen = ?, age_sportsmen = ?, date_passing = ?, aerobic_p_legs = ?, aerobic_p_args = ?, heart_rate_aerobic_legs = ?, heart_rate_aerobic_args = ?, anaerobic_p_legs = ?, anaerobic_p_args = ?, heart_rate_anaerobic_legs = ?, heart_rate_anaerobic_args = ?, mpk_legs = ?, mpk_args = ?, yoc_max_legs = ?, yoc_max_args = ?;";
